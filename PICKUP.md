...
- [x] `join_engagement` blocking dial — `import_ticket` now wrapped in 10s timeout
- [x] Blobs redb lock — `FsStore::load` now wrapped in 5s timeout; falls back to in-memory
- [x] Presence heartbeat no cancel on leave — `JoinHandle` stored, aborted on `leave()`
- [x] Member list never populates — client-side last-seen map in `MeshManager` from presence events
