"""Worker loop: a stripped headless agent loop for subagent tasks.

A worker is dispatched by DispatchWorkerTool. It runs an isolated conversation
Context with a worker-specific system prompt, streams from a cheap worker
Provider, executes tools with headless-style gating, and reports a concise
WorkerResult.
"""
...
if TYPE_CHECKING:
    ...
    from riftor.workers.registry import WorkerSpec

#: The dispatch tool is excluded from the worker tool set so a worker can never
#: spawn its own workers (no recursion).
DISPATCH_TOOL_NAME = "dispatch_worker"


@dataclass
class WorkerResult:
    """The outcome of one worker."""
...
async def run_worker(
    ...
    *,
    ...
    """Run one worker on ``task``. Never raises — failures become WorkerResult.error."""
    result = WorkerResult(task=task)
    ctx = Context(workdir=toolctx.workdir)
    if worker_spec.prompt:
        ctx.add_user(f"[Worker role: {worker_spec.name}]\n\n{worker_spec.prompt}")
    ctx.add_user(task)
    ...
    try:
        for _ in range(max_steps):
            ...
            for call in turn.tool_calls:
                ...
                content = await _run_worker_tool(
                ...
                if eng is not None:
                    eng.auto_tick_methodology(call.name, content[:500])
...
def _detail_label(call: ToolCall) -> str:
    """A short live-activity label for a worker tool call."""
    if call.name == "bash":
...
# Backward-compat aliases for tests migrating incrementally
ChaklaResult = WorkerResult
run_chakla = run_worker
_run_chakla_tool = _run_worker_tool
