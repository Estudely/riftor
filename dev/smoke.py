...
async def main() -> None:
    ...
    async with app.run_test() as pilot:
        ...
        # /config opens the prompt-owned inline picker (not a modal). Verify it
        # mounts above the prompt, exposes keyboard-filterable settings, and
        # Esc closes it without replacing the main screen.
        ...
        setting_rows = [
            row for row in picker.query(".config-setting-row") if row.display
        ]
        assert setting_rows, "inline picker must show root settings"
        labels = " ".join(str(row.render()) for row in setting_rows)
        for expected in ("Main model", "Temperature", "Worker model", "Theme"):
            assert expected in labels, labels

        # Typing filters without opening the slash-command dropdown.
        inp.value = "theme"
        ...
        assert "Theme" in str(visible[0].render())

        await pilot.press("escape")
        await pilot.pause()
        assert not picker.has_class("active")
